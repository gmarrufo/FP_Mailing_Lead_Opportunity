    ...
    public class PreLeadCreate : IPlugin
    {
        ...
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            if (context.InputParameters.Contains("Target") && context.InputParameters["Target"] is Entity)
            {
                                ...
                                if (!Convert.ToBoolean(entity.Attributes["pearl_retention"]))
                                {
                                    ...
                                    if (item.Attributes["pearl_retention"] != null)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            dDaysToExpire = Convert.ToDateTime(item.Attributes["createdon"]).AddDays(Convert.ToDouble(item.Attributes["pearl_daystoexpire"]));
                        ...
                        if (!(distributed))
                        {
                            ...
                            else
                            {
                                if (entity.Attributes.Contains("pearl_routingtype") && entity.Attributes.Contains("pearl_leadtype"))
                                {
                                    ...
                                    //eText += entityLead.pearl_pearl_leadroutingtype_lead_RoutingType.pearl_name;
                                    // Query Expression to get from Lead Routing Type the Default Distribution Account based on pearl_leadRoutingType
                                    eText = "0.9 ";
                                    /*if (entityLead.pearl_RoutingType == null)
                                        eText += "null"; 
                                    else
                                        eText += "not Null"; */
                                    /*if (entity.Attributes["pearl_routingtype"] == null)
                                        eText += "ent null";
                                    else
                                        eText += "ent not Null";*/

                                    LeadRoutingTypeDefaultAccount lRTDA = new LeadRoutingTypeDefaultAccount();
                                    entityCollection1 = lRTDA.Execute(serviceProvider, (EntityReference)entity.Attributes["pearl_routingtype"]);// entityLead.pearl_RoutingType);//entityLead.Attributes["pearl_routingtype"]);
                                    eText = "1.0";
                                    Entity entLeadRoutingType = service.Retrieve("pearl_leadroutingtype", ((EntityReference)entity.Attributes["pearl_routingtype"]).Id, new ColumnSet(true));//entityLead.pearl_RoutingType.Id, new ColumnSet(true));
                                    pearl_leadroutingtype plrt = entLeadRoutingType.ToEntity<pearl_leadroutingtype>();
                                    eText = "1.1";
                                    Entity entDefaultContact = service.Retrieve("contact", plrt.pearl_DefaultContact.Id, new ColumnSet(true));
                                    Contact defaultContact = entDefaultContact.ToEntity<Contact>();
                                    eText = "1.2";
                                    EntityReference sDefaultAccount = plrt.pearl_DefaultAccount;//(EntityReference)entityCollection1[0].Attributes["pearl_defaultaccount"];
                                    ...
                                    foreach (Contact cT in entityCollection2)
                                    {
                                        ...
                                        int intDiff = (int)t.TotalMilliseconds;
                                        //eText += " now: " + dToday.ToString() + " orig " + cT.pearl_LastOpportunityAssignment.ToString() + " dif(double) " + t.TotalMilliseconds.ToString() + " dif(int) " + intDiff + " ";
                                    ...
                                    QueryByAttribute queryContact1 = new QueryByAttribute
                                    {
                                        ...
                                        ColumnSet = new ColumnSet("parentcustomerid", "pearl_lastopportunityassignment", "pearl_assignlead", "adx_systemuserid")
                                    };
                                    e = 3111;
                                    queryContact1.AddAttributeValue("contactid", sDefaultContact.Id.ToString());
                                    e = 3112;
                                    ...
                                    if (sContactID != "")
                                    {
                                        ...
                                        Entity entAssignContact = service.Retrieve("contact", entResultContact1[0].Id, new ColumnSet(true));
                                        Contact assignContact = entAssignContact.ToEntity<Contact>();
                                        if (entResultContact1[0].Attributes.Contains("adx_systemuserid"))
                                            if (entResultContact1[0].Attributes["adx_systemuserid"] != null)
                                            {
                                                Entity entSysUser = service.Retrieve("systemuser", ((EntityReference)entResultContact1[0].Attributes["adx_systemuserid"]).Id, new ColumnSet(true));
                                    ...
                                    else
                                    {
                                        ...
                                        Entity entSysUser = service.Retrieve("systemuser", defaultContact.adx_systemuserid.Id, new ColumnSet(true));
                                        sysUser = entSysUser.ToEntity<SystemUser>();
                                        entity.Attributes["ownerid"] = sysUser.ToEntityReference();
                                        entity.Attributes["pearl_ddr"] = true;
                                        entity.Attributes["pearl_distributed"] = true;
                ...
                {
                    throw new InvalidPluginExecutionException("** Error in position: " + e + " ** ***Etext: "+ eText +"*** An error occurred in the InsertLeadProcess plug-in.", ex);
                }
                catch (Exception ex)
                {
                    tracingService.Trace("** Error in position: " + e + " ** ***Etext: " + eText + "*** InsertLeadProcess: {0}", ex.ToString());
                    throw;
                }
            }
        }
        ...
        */
            ...
            )
        {
            ...
            {
                throw new InvalidPluginExecutionException("** Error in position: " + e + " **  An error occurred in the InsertLeadProcess plug-in.", ex);
            }
            catch
            {
                throw;
