    ...
    public class InsertLeadProcess : IPlugin
    {
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            if (context.InputParameters.Contains("Target") && context.InputParameters["Target"] is Entity)
            {
                ...
                {
                    throw new InvalidPluginExecutionException("** Error in position: " + e + " ** ***Etext: " + eText + "*** An error occurred in the UpdateQualifyLeadProcess plug-in.", ex);
                }
                catch (Exception ex)
                {
                    tracingService.Trace("** Error in position: " + e + " ** ***Etext: " + eText + "*** UpdateQualifyLeadProcess: {0}", ex.ToString());
                    throw;
                }
            }
        }
